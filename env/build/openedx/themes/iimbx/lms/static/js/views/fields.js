    ...
    define([
        ...
        FieldViews.FieldView = Backbone.View.extend({
            ...
            indicators: {
                ...
                error: HtmlUtils.joinHtml(
                    HtmlUtils.HTML('<em class="fa fa-exclamation-triangle message-error" aria-hidden="true"></em><span class="sr">'),  // eslint-disable-line max-len
                    gettext('Error'),
                    HtmlUtils.HTML('</span>')
                ),
                validationError: HtmlUtils.joinHtml(
                    HtmlUtils.HTML('<em class="fa fa-exclamation-triangle message-validation-error" aria-hidden="true"></em><span class="sr">'),  // eslint-disable-line max-len
                    gettext('Validation Error'),
                    HtmlUtils.HTML('</span>')
                ),
                inProgress: HtmlUtils.joinHtml(
                    HtmlUtils.HTML('<em class="fa fa-spinner fa-pulse message-in-progress" aria-hidden="true"></em><span class="sr">'),  // eslint-disable-line max-len
                    gettext('In Progress'),
                    HtmlUtils.HTML('</span>')
                ),
                success: HtmlUtils.joinHtml(
                    HtmlUtils.HTML('<em class="fa fa-check message-success" aria-hidden="true"></em><span class="sr">'),  // eslint-disable-line max-len
                    gettext('Success'),
                    HtmlUtils.HTML('</span>')
                ),
                plus: HtmlUtils.joinHtml(
                    HtmlUtils.HTML('<em class="fa fa-plus placeholder" aria-hidden="true"></em><span class="sr">'),
                    gettext('Placeholder'),
                    HtmlUtils.HTML('</span>')
                )
            },
        ...
        FieldViews.ReadonlyFieldView = FieldViews.FieldView.extend({
            ...
            updateValueInField: function() {
                // TODO: Create separate view for profile image
                if(this.options.title == 'Username'){
                    $('.acc-profileimg img').attr('src', this.model.profileImageUrl());
                }
        ...
        FieldViews.TextFieldView = FieldViews.EditableFieldView.extend({
            ...
            updateValueInField: function() {
                ...
                if (value){
                    this.$('.u-field-value input').triggerHandler('focus');
                }
            },
        ...
        FieldViews.DropdownFieldView = FieldViews.EditableFieldView.extend({
            ...
            updateValueInField: function() {
                ...
                if (this.modelValue() || ''){
                    this.$('.u-field-value select').select2().val(this.modelValue());
                } else {
                    this.$('.u-field-value select').select2({
                        placeholder: this.options.title
                    })
                }
            },
